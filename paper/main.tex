% LatentShift: Mitigating Catastrophic Forgetting via Hilbert-Inspired Latent Shifting
...
% ---------------------------------------------------------------
\title{LatentShift: Mitigating Catastrophic Forgetting\\via Hilbert-Inspired Latent Shifting}
...
We propose \textbf{LatentShift}, a \emph{replay-free} method that protects
learned representations via orthogonal projection in a shared latent space. After each task, LatentShift identifies the
...
LatentShift achieves consistently among the \emph{lowest backward transfer (BWT)} across benchmarks, particularly on challenging datasets such as Split-CIFAR-100 and Split-TinyImageNet, outperforming even replay-based methods like DER++ in forgetting
...
Drawing loose inspiration from Hilbert's Hotel---where an infinite hotel
accommodates new guests by shifting existing ones---LatentShift archives
old task representations into a protected subspace via SVD and frees
orthogonal dimensions for new learning. We emphasize that this is a
motivational metaphor rather than a mechanistic description: the actual
method relies on orthogonal projection in a finite-dimensional latent space,
and the capacity is bounded (\Cref{prop:capacity}).
...
\begin{enumerate}
    ...
    \item Extensive experiments across six benchmarks (including a 50-task
          ...
          and three seeds per
          configuration. LatentShift-Tuned achieves consistently among the \emph{lowest backward
          transfer} (least forgetting) across benchmarks---particularly on
          challenging datasets like CIFAR-100 and TinyImageNet, where it
          outperforms even replay-based methods---while maintaining
...
\paragraph{Motivational analogy.}
The name ``LatentShift'' draws loose inspiration from Hilbert's Grand Hotel
paradox, where an infinite hotel accommodates new guests by shifting existing
ones. In our finite-dimensional setting, ``rooms'' correspond to latent
dimensions and ``guests'' to task representations; the SVD-based archival
followed by orthogonal projection plays the role of the shift operator.
However, unlike Hilbert's Hotel (which has countably infinite rooms), our
latent space has finite dimension~$d$, so capacity is bounded
(\Cref{prop:capacity}). The analogy is purely motivational---the method's
guarantees follow from orthogonal projection, not from any infinite-capacity
argument.
...
\paragraph{Strong forgetting prevention across benchmarks.}
LatentShift-Tuned achieves consistently among the \emph{lowest
BWT} (least forgetting) across benchmarks, particularly on the more
challenging datasets. On Split-CIFAR-100, LS-Tuned's BWT of $-4.1\%$ is less than
half that of DER++ ($-9.3\%$), despite DER++ using a replay buffer.
On TinyImageNet, the gap widens further: $-4.4\%$ vs.\ $-12.9\%$.
On Split-MNIST and Permuted-MNIST, HAT achieves BWT~$= 0.0\%$ through hard
attention masks, while LS-Tuned achieves $-0.3\%$ and $-0.5\%$ respectively---both
near-zero. On Split-CIFAR-10, PackNet achieves positive transfer ($+4.4\%$ BWT)
due to its pruning-based isolation, while LS-Tuned achieves $-4.2\%$.
On the more challenging CIFAR-100 and TinyImageNet benchmarks, LS-Tuned
achieves the lowest BWT of any method, confirming that latent-space
orthogonal projection provides particularly strong forgetting prevention
on complex, longer task sequences.
...
\input{figures/seq_cifar100_table}
...
$49.6\%$ accuracy with $-6.8\%$ BWT on ViT-Tiny---the lowest forgetting
among all ViT methods---while LS-Tuned reaches $51.0\%$ accuracy with
$-12.3\%$ BWT. The tuned variant's higher forgetting on ViT-Tiny (compared to
DER++'s $-9.9\%$ BWT) reflects that the expanded latent dimension ($d=1024$)
may over-parameterize the ViT-Tiny bottleneck, suggesting that
architecture-specific tuning of the latent dimension is beneficial.
...
\input{figures/ci_table}

\subsection{Computational Cost}

\Cref{tab:cost} reports wall-clock training times. LatentShift is among the
most efficient methods: 46s on MNIST, 1,197s on CIFAR-10, and 2,244s on
CIFAR-100. In contrast, GPM requires 13,073s on CIFAR-100 due to its
per-layer SVD operations, and HAT requires 23,478s on TinyImageNet. The
...
consistently among the lowest backward transfer (BWT) across benchmarks (\Cref{tab:bwt}),
particularly on the challenging CIFAR-100 and TinyImageNet datasets where it
surpasses all methods including replay-based ones. On CIFAR-100,
...
for DER++. On simpler benchmarks (Split-MNIST, Permuted-MNIST), HAT achieves
zero forgetting through hard attention masks, while LS-Tuned's near-zero BWT
($-0.3\%$, $-0.5\%$) remains competitive. This validates the theoretical prediction: orthogonal projection in
...
computational overhead (GPM: 13,073s on CIFAR-100 vs.\ LS: 2,244s), fragments
...
On Split-CIFAR-100, LatentShift-Tuned achieves lower BWT ($-4.1\%$) than all
prompt baselines (L2P $-10.0\%$, DualPrompt $-19.4\%$, CODA-P $-8.7\%$),
while the prompt methods show higher forgetting due to interference in the
shared prompt pool.
...
mitigates catastrophic forgetting by orthogonal projection in a shared latent
space. LatentShift archives task representations via SVD and frees
complementary directions for new learning.
...
achieves consistently among the lowest backward transfer (least forgetting)
across benchmarks, outperforming replay-based methods on challenging
datasets like CIFAR-100 and TinyImageNet, while
...
Mitigating catastrophic forgetting has significant implications for deployed
continual learning systems. On the positive side, methods like LatentShift
enable models to accumulate knowledge over time without requiring access to
previous training data, which is beneficial for privacy-sensitive domains
(e.g., medical imaging, personal assistants) and resource-constrained edge
deployments where storing past data is infeasible. However, improved
forgetting prevention also enables longer autonomous operation with less
human oversight, which may be undesirable in safety-critical applications
where periodic retraining serves as a natural checkpoint for human review.
We encourage practitioners to maintain appropriate monitoring and evaluation
protocols even when deploying models with strong forgetting prevention.
From a computational perspective, LatentShift's efficiency (operating on a
single latent space rather than per-layer) reduces the energy cost of
continual learning relative to methods like GPM, though any additional
training beyond a single model still incurs environmental cost.

\paragraph{Code and data availability.}
Code and experiment configurations will be released upon acceptance.
All datasets used (MNIST, CIFAR-10, CIFAR-100, Tiny ImageNet) are publicly
