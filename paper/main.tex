...
ViT-Tiny encoders---with 11~methods and 3~seeds per configuration show that
...
\begin{enumerate}
    ...
    \item Extensive experiments across six benchmarks (including a 50-task
          sequence), two encoder architectures (ResNet-18, ViT-Tiny), ten
          baselines spanning four CL families, and three seeds per
...
\paragraph{Baselines.}
We compare against ten baselines spanning four CL families:
...
\paragraph{Comparison with modern methods.}
...
achieve strong class-incremental accuracy on standard benchmarks using
pre-trained Vision Transformers (e.g., ViT-B/16 on ImageNet-21k). However,
these methods operate in a fundamentally different regime: they fine-tune
prompts rather than full networks, and their backbone features are already
highly discriminative. In contrast, LatentShift trains networks
\emph{from scratch}---a more challenging and general setting shared with GPM,
TRGP, PackNet, and HAT. Our ViT-Tiny experiments (\Cref{tab:vit}) demonstrate
that LatentShift does transfer to transformer architectures, achieving the
lowest BWT ($-6.8\%$) among all methods. Architecture-expansion methods such as
